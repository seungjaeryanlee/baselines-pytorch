...
def test_TorchWrapper_reset_type():
    """
    Test if environment wrapped in TorchWrapper returns a `torch.Tensor` when `reset()` is called.
    """
...
def test_TorchWrapper_reset_dtype():
    """
    Test if environment wrapped in TorchWrapper returns a `torch.Tensor` state with `torch.float32` datatype when `reset()` is called.
    """
    env = gym.make('CartPole-v0')
    env = TorchWrapper(env)
...
def test_TorchWrapper_reset_shape():
    """
    Test if environment wrapped in TorchWrapper returns a `torch.Tensor` of the correct shape.
    """
    env = gym.make('CartPole-v0')
...
def test_TorchWrapper_step_type():
    """
    Test if environment wrapped in TorchWrapper returns `next_state, reward, done` with `torch.Tensor` types when `step()` is called.
    """
    env = gym.make('CartPole-v0')
    env = TorchWrapper(env)
    env.reset()
...
def test_TorchWrapper_step_dtype():
    """
    Test if environment wrapped in TorchWrapper returns `next_state, reward, done` with correct dtypes when `step()` is called.
    """
    env = gym.make('CartPole-v0')
    env = TorchWrapper(env)
    env.reset()
...
def test_TorchWrapper_step_shape():
    ...
    next_state, reward, done, info = env.step(env.action_space.sample())
    w_next_state, w_reward, w_done, info = wrapped_env.step(wrapped_env.action_space.sample())
